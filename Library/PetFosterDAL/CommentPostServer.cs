...
namespace PetFoster.DAL
{
    public class CommentPostServer
    {
        ...
        /// <returns></returns>
        public static DataTable CommentPostInfo(decimal Limitrows = -1, string Orderby = null)
        {
            ...
            using (OracleConnection connection = new OracleConnection(conStr))
            {
                ...
                string query = "SELECT user_id,post_id,TO_CHAR(comment_time,'YYYY-MM-DD') as comment_date, TO_CHAR(comment_time,'HH24:MI:SS')as commented_time,comment_contents  FROM comment_post";
                if (Limitrows > 0)
        ...
        public static bool GetCommentPostEntry(string UID, string PID)
        {
            bool con = false;
            User user1 = new User();
            using (OracleConnection connection = new OracleConnection(conStr))
            {
                ...
                command.CommandText = "select *from comment_post where Post_ID=:post_id and User_ID=:user_id";
        ...
        /// <param name="UID"></param>
        /// <param name="PID"></param>
        public static void InsertCommentPost(string UID, string PID)
        {
            // 添加新行
            try
            {
                using (OracleConnection connection = new OracleConnection(conStr))
                {
                    ...
                    OracleCommand command = connection.CreateCommand();
                    command.CommandType = CommandType.Text;
                    command.CommandText = "INSERT INTO comment_post (user_id, post_id) " +
